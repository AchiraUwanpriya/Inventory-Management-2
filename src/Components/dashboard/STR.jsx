...
const STR = () => {
  ...
  // Fetch transactions & products on mount
  useEffect(() => {
    dispatch(GetStockTransactionsByFormType('STR'));
    dispatch(GetAllProducts());
  }, [dispatch]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editingTransactionID) {
        await dispatch(PutStockTransactionsDetails(payload));
        setEditingTransactionID(null);
        showPopupMessage("✅ Stock Transfer updated successfully!");
      } else {
        await dispatch(AddStockTransactionsDetails(payload));
        showPopupMessage("✅ Stock Transfer added successfully!");
      }

      await dispatch(GetStockTransactionsByFormType('STR'));
      ...
      showPopupMessage("❌ Failed to save Stock Transfer!", true);
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div style={styles.paginationContainer}>
        ...
        <div style={styles.pageInfo}>
          Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredTransactions.length)} of {filteredTransactions.length} Stock Transfers
        </div>
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div style={styles.headerContent}>
          <Truck size={isMobile ? 36 : 52} color={colors.white} />
          <div style={{flex: 1}}>
            <h1 style={styles.pageTitle}>STOCK TRANSFERS (STR)</h1>
      ...
      <div style={styles.controls}>
        <div style={styles.searchBox}>
          ...
          <input
            type="text"
            placeholder="Search Transfers by ID, product, or reference..."
            value={searchTerm}
            onChange={e => setSearchTerm(e.target.value)}
            style={styles.searchInput}
            onFocus={(e) => {
        ...
        <button
          ...
          disabled={isSubmitting}
        >
          <Plus size={isMobile ? 20 : 24} /> Add Transfer
        </button>
      </div>
      ...
      {showForm && (
        <div style={styles.modalOverlay}>
          <div style={{
            ...
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>
                {editingTransactionID ? "EDIT TRANSFER" : "ADD NEW TRANSFER"}
              </h3>
              <button 
                style={styles.closeButton}
                onClick={() => {
              ...
              <form onSubmit={handleFormSubmit}>
                ...
                <div style={styles.buttonGroup}>
                  ...
                  <button 
                    ...
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? "Saving..." : (editingTransactionID ? "Update Transfer" : "Add Transfer")}
                  </button>
                </div>
              </form>
            </div>
      ...
      <div style={styles.tableContainer}>
        <table style={styles.table}>
          <thead style={styles.tableHeader}>
            <tr>
              <th style={styles.tableHeaderCell}>Transfer ID</th>
              <th style={styles.tableHeaderCell}>Product Name</th>
              <th style={styles.tableHeaderCell}>Quantity</th>
              <th style={styles.tableHeaderCell}>Balance Quantity</th>
              <th style={styles.tableHeaderCell}>Type</th>
            ...
            {currentItems.length === 0 ? (
              <tr>
                <td colSpan={7} style={{...styles.tableCell, textAlign: 'center', padding: isMobile ? '40px' : '60px'}}>
                  <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', color: colors.gray}}>
                    <Package size={isMobile ? 48 : 72} color={colors.grayLight} />
                    <p style={{fontSize: isMobile ? '18px' : '20px', margin: 0, fontWeight: '700', color: colors.grayDark}}>No Stock Transfers found</p>
                    <p style={{fontSize: isMobile ? '12px' : '14px', margin: 0}}>Try adjusting your search criteria or filters</p>
                  </div>
                </td>
              </tr>
